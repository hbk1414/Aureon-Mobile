...
export function MicroInvesting() {
  ...
  const handleInvestment = async (fundId: string) => {
    ...
    try {
      ...
      const fundName = INVESTMENT_FUNDS.find((f) => f.id === fundId)?.name || "your chosen fund"
      setInvestmentComplete(true)
      setShowInvestmentModal(false)

      toast({
        title: "Investment Complete!",
        description: `Successfully invested £${totalAvailable.toFixed(2)} in ${fundName}`,
      })
    } catch (error) {
      toast({
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-2">
            <Switch id="round-up-toggle" checked={roundUpEnabled} onCheckedChange={setRoundUpEnabled} />
          ...
          <>
            <div className="space-y-6">
              <div className="bg-emerald-50 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold text-emerald-600">£{totalAvailable.toFixed(2)}</div>
                <p className="text-sm text-emerald-700 mt-1">From recent round-ups</p>
              </div>
              ...
              {totalAvailable > 0 && (
                ...
                >
                  ...
                  {isInvesting ? "Processing..." : "Invest Spare Change"}
                </Button>
              )}
                ...
                <div className="mt-4 p-3 bg-blue-100 rounded text-xs text-blue-800">
                  <div className="flex justify-between items-center">
                    <span>
                      ...
                      {Object.values(fundInvestments)
                        .reduce((a: any, b: any) => (Number(a) || 0) + (Number(b) || 0), 0)
                        .toFixed(2)}
                    </span>
                    {totalAvailable > 0 && (
                      <span>
                        <strong>Available to Invest:</strong> £{totalAvailable.toFixed(2)}
                      </span>
                    )}
                  </div>
                </div>
                    ...
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="text-right">
                        <div className="font-medium text-emerald-600">
                          +£{transaction.roundUp?.toFixed(2) || "0.00"}
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="py-4 space-y-4">
            <p className="text-sm text-gray-600">
              Select which fund to invest your £{totalAvailable.toFixed(2)} spare change into:
            </p>
